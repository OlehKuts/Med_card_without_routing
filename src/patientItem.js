...
export const PatientItem = ({
  ...
  const numArrayCreator = (first, quantity) => {
    let arr = [];
  ...
  const lastYearList = numArrayCreator(2020, 2040);
  const [lastYear, setLastYear] = useState(Number(initDate.getFullYear()));
  ...
  const [showTownLine, setShowTownLine] = useState(false);
  const [showVillageLine, setShowVillageLine] = useState(false);
  const [showOtherRegionLine, setShowOtherRegionLine] = useState(false);
  const onShowTownLine = () => {
    setShowTownLine(true);
    setShowVillageLine(false);
    setShowOtherRegionLine(false);
  };
  const onShowVillageLine = () => {
    setShowTownLine(false);
    setShowVillageLine(true);
    setShowOtherRegionLine(false);
  };
  const onShowOtherRegionLine = () => {
    setShowTownLine(false);
    setShowVillageLine(false);
    setShowOtherRegionLine(true);
  };
  ...
  const [finalDiagnosis, setFinalDiagnosis] = useState(patient.diagnosis);
  ...
  const birthYearList = numArrayCreator(2002, 2020);
  ...
  return (
    <>
      ...
      {showExtractForm && (
        <div id="extractForm">
          <p className="flexi">
            <div className="label">Тип населеного пункту</div>
            <button onClick={onShowTownLine}>Місто</button>
            <button onClick={onShowVillageLine}>Село</button>
            <button onClick={onShowOtherRegionLine}>Інша область</button>
          </p>
            ...
            {showOtherRegionLine && (
              <p className="flexi">
                <input
                  ...
                  placeholder="введіть повну адресу проживання..."
                />
              </p>
            )}
